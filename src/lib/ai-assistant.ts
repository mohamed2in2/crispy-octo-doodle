...
- لديك صلاحية كاملة لرؤية كل بيانات الطالب: درجاته، تقدمه، كورساته، نقاط ضعفه، وملاحظاته
- تعرف الطالب جيداً وتتحدث معه كمرشد شخصي يفهم وضعه
- أنت جزء من الموقع، لست مجرد بوت - تساعد الطالب في استخدام كل الأدوات المتاحة

ما تقدر تعمله:
1. **خطة دراسية مخصصة** - بناءً على نقاط ضعف الطالب الحقيقية
2. **تحليل الأداء** - تشرح للطالب فين هو ضعيف وليه
3. **طلب تعديل درجة** - لو الطالب أكد إن في إجابة صحيحة وحُسبت غلط، اعمل GradeAdjustmentRequest (المدرس هو اللي يوافق)
4. **استلام شكاوى** - لو الطالب مش مرتاح من المدرس أو الكورس، سجلها كـ StudentFeedback
5. **خطة بديلة** - لو الطالب بياخد الكورس مع حد تاني، اعمله خطة مختلفة بدون الفيديوهات اللي شافها برة
6. **توجيه** - وجّه الطالب لصفحة الكورس أو المكتبة المناسبة
7. **تذكرة دعم** - لو فيه مشكلة فنية أو محتوى غلط، اعمل SupportTicket

قواعد مهمة جداً:
- **لا تعدّل الدرجات مباشرة** - فقط أنشئ طلب تعديل وانتظر موافقة المدرس
- **لا تعتمد طلبات تعديل تافهة** - تأكد إن الطالب فعلاً عنده سبب قوي (إجابة صحيحة لكن متسجلة غلط، أو سؤال غامض)
- **كن صادقاً** - لو الطالب فعلاً ضعيف في موضوع، قوله بأدب، متجاملوش
- **اقترح حلول عملية** - مش مجرد كلام عام
- **استخدم بيانات الطالب** - قوله "شفت إنك جبت 40% في كويز كذا" مش "حاول تذاكر"

الرد يجب أن يكون JSON بالشكل ده:
{
  "message": "ردك للطالب باللغة العربية",
  "actions": [
    {
...
function summarizeContext(ctx: StudentContext): string {
  ...
  return `بيانات الطالب الكاملة:

الملف الشخصي:
- الاسم: ${ctx.profile.name}
- العمر: ${ctx.profile.age ?? "غير محدد"}
- المرحلة الدراسية: ${ctx.profile.educationalStage ?? "غير محددة"}
...
function buildChatContextForStaff(history: ChatMessage[], ctx: StudentContext): { chatHistory: string; studentInfo: string } {
  const chatHistory = history.slice(-8).map((m) =>
    `${m.role === "user" ? "الطالب" : "المرشد"}: ${m.content.replace(/\[م:[^\]]+\]/g, "").trim()}`
  ).join("\n");
  const studentInfo = [
    `الاسم: ${ctx.profile.name}`,
...
function fallbackResponse(
  ...
  if (state === "3b" && data) {
    if (input.length < 20) {
      ...
      message = `✅ تم إرسال طلب تعديل الدرجة للمدرس!\n\n📋 السبب: ${input.slice(0, 80)}${input.length > 80 ? "..." : ""}\n\n⏳ المدرس هيراجع طلبك. اكتب 5 لمتابعة الحالة.\n\nاكتب 0 للرجوع\n\n[م:menu]`;
    }
    return { message, actions, source: "fallback" };
  }
...
هل إجابة الطالب فعلاً غلط؟ أو ممكن تكون صحيحة من ناحية أخرى؟ ممكن السؤال يكون غامض؟
...
رد بـ JSON:
{
  ...
  return {
    wasMisgraded: false,
    reasoning: "لم نتمكن من تحليل السؤال تلقائياً، يرجى مراجعة المدرس",
    confidence: 0,
  };
}
  ...
  const messages: ChatMessage[] = [
    {
      role: "system",
      content:
        "أنت محلل تعليمي. حلل بيانات الطالب واستخرج 3-5 رؤى مهمة (نقاط قوة، نقاط ضعف، توصيات، تحذيرات). رد بـ JSON.",
    },
    {
      role: "user",
