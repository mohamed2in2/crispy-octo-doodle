...
export function StreakFlame({ role }: { role?: string }) {
  ...
  useEffect(() => {
    ...
    fetch("/api/student/stats", { credentials: "include" })
      ...
      .then((data: { streak?: number } | null) => {
        if (alive && data) setStreak(data.streak ?? 0);
      })
      .catch(() => {});
  ...
  const nativeShare = async () => {
    ...
    try {
      await navigator.share({ title: "Code-UP", text: message, url });
      setOpen(false);
    } catch {
      // The share sheet may be dismissed without completing an action.
    }
  };
    ...
    <>
      ...
      >
        <span className={active ? "streak-flame" : ""} style={{ display: "inline-flex", transformOrigin: "50% 70%" }}>
          <FlameIcon color={streakMeta.color} glow={active ? streakMeta.glow : undefined} />
        </span>
        <span className="text-sm font-black tabular-nums" style={{ color: streakMeta.color }}>{streak}</span>
        <style jsx>{`
        ...
        <AnimatePresence>
          {open && (
            ...
            >
              ...
              >
                <button onClick={() => setOpen(false)} aria-label="إغلاق" className="absolute top-3 left-3 flex h-8 w-8 items-center justify-center rounded-lg text-[var(--ink-muted)] transition-colors hover:bg-[var(--border)] hover:text-[var(--ink)]">
                  <svg className="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round" aria-hidden>
                    <path d="M18 6L6 18M6 6l12 12" />
                  </svg>
                </button>

                <div className="mx-auto mb-3 flex h-20 w-20 flex-col items-center justify-center rounded-2xl" style={{ background: active ? `${streakMeta.color}1f` : "var(--border)" }}>
                  <FlameIcon color={streakMeta.color} size={34} glow={active ? streakMeta.glow : undefined} />
                </div>
                <p className="text-2xl font-black" style={{ color: streakMeta.color }}>{streak} {streak === 1 ? "يوم" : "أيام"}</p>
                <p className="mt-1 mb-5 text-sm leading-relaxed text-[var(--ink-muted)]">{active ? `سلسلة مواظبة متتالية — ${streakMeta.label}` : "ابدأ سلسلتك اليوم وشاركها!"}</p>

                {canNativeShare && (
                  <button onClick={nativeShare} className="mb-3 flex w-full items-center justify-center gap-2 rounded-xl bg-sky-500 py-2.5 text-sm font-bold text-white transition-colors hover:bg-sky-400">
                    <svg className="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round" aria-hidden>
                ...
                <div className="mb-3 grid grid-cols-4 gap-2">
                  {targets.map((target) => (
                    <button
                      key={target.id}
                      onClick={() => openWin(target.href)}
                      title={target.label}
                      className="flex flex-col items-center gap-1.5 rounded-xl border border-[var(--border)] py-2.5 transition-colors hover:bg-[var(--border)]"
                    >
                      <svg className="h-6 w-6" viewBox="0 0 24 24" fill={target.color} aria-hidden>{target.icon}</svg>
                      <span className="text-[10px] font-semibold text-[var(--ink-muted)]">{target.label}</span>
                    </button>
                  ))}
                </div>

                <button onClick={copy} className="flex w-full items-center justify-center gap-2 rounded-xl border border-[var(--border)] py-2.5 text-sm font-semibold text-[var(--ink-muted)] transition-colors hover:border-[var(--ink-muted)]/40 hover:text-[var(--ink)]">
                  {copied ? (
                    <>
                      <svg className="h-4 w-4 text-emerald-500" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2.5} strokeLinecap="round" strokeLinejoin="round" aria-hidden>
