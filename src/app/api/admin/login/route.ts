...
export async function POST(req: NextRequest) {
  ...
  if (previousSession?.role === "superadmin") {
    try {
      await logAdminAction({
        adminId: previousSession.id,
        adminName: previousSession.name,
      ...
      // Login remains available if optional observability fails.
    }
  }
  ...
  try {
    ...
    const current = await prisma.appSetting.findUnique({ where: { key: failedTriesKey } });
    const currentTries = current ? Number.parseInt(current.value, 10) || 0 : 0;
    if (currentTries >= 30) {
      return NextResponse.json({ error: "تم تجاوز الحد الأقصى لمحاولات الدخول الفاشلة اليوم" }, { status: 429 });
    }
    ...
    if (role === "superadmin") {
      ...
      // The emergency password may select an owner account, but never creates a
      // database-less session. Disabling/deleting the owner therefore revokes it.
      if (verifyMasterPassword(password)) {
        const owner = await prisma.user.findFirst({
          where: { role: "superadmin", isOwner: true, isActive: true, isDeleted: false },
        });
        if (!owner) {
          return NextResponse.json({ error: "حساب المالك غير مهيأ. استخدم حساب مشرف عام نشط." }, { status: 503 });
        }
