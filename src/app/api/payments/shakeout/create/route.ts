...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const { number, amount, method, courseTitle } = body as { number?: string; amount?: number; method?: string; courseTitle?: string };
    if (!amount || amount < 5) return NextResponse.json({ error: "المبلغ مطلوب (الحد الأدنى 5 جنيه)" }, { status: 400 });
    ...
    const result = await createShakeOutPayment({ number: number || "", amount: totalAmount, method: selectedMethod, client: session.id, details, webhook_url: webhookUrl });
    ...
    if (reference) {
      ...
      await prisma.balanceTransaction.create({ data: { userId: session.id, type: SHAKEOUT_PENDING_TYPE, amount: totalAmount, note: noteText } });
