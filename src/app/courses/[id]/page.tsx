...
export default function CourseLearningPage() {
  ...
  const allVideos = useMemo(() => {
    return course?.folders.flatMap((folder) => folder.videos) ?? [];
  }, [course]);

  const allQuizzes = useMemo(() => {
    return course?.folders.flatMap((folder) => folder.quizzes) ?? [];
  }, [course]);

  const selectedVideo = allVideos.find((video) => video.id === selectedVideoId) ?? null;
  const selectedQuiz = allQuizzes.find((quiz) => quiz.id === selectedQuizId) ?? null;
  ...
  const loadCourse = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        if (res.status === 403) {
          setError("ليس لديك صلاحية لهذا الكورس. فعّل كود الكورس أولاً.");
          setLoading(false);
          return;
        }
      ...
      nextCourse.folders.forEach((folder, index) => {
        initialCollapse[folder.id] = index !== 0;
      });
  ...
  useEffect(() => {
    const loadSecureVideo = async () => {
      ...
      try {
        const res = await fetch(`/api/videos/${selectedVideoId}/secure-url`, {
          credentials: "include",
        });
        const data = await res.json();
            ...
            <section className="lg:col-span-2 space-y-6">
              <div className={`relative overflow-hidden rounded-[2rem] p-6 sm:p-8 border border-white/50 dark:border-white/10 shadow-[0_30px_80px_-35px_rgba(15,23,42,0.5)] bg-gradient-to-r ${theme.panel} text-white`}>
                ...
                <div className="relative">
                  ...
                  <div className="flex flex-wrap items-center gap-2 mt-3">
                    ...
                    <span className="text-xs px-2.5 py-1 rounded-full bg-white/15 border border-white/15">⏱️ مدة الاختبار مخصصة لكل اختبار</span>
                  </div>
                  ...
                  <div className="mt-5 flex flex-wrap gap-2 text-xs text-white/85">
                    ...
                    <span className="rounded-full bg-white/10 px-3 py-1">محتوى منظم لكل مرحلة</span>
                  </div>
                </div>
              </div>
              ...
              <div className="flex items-center gap-2 rounded-3xl p-2 border border-white/70 dark:border-white/10 bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl shadow-lg">
                <button
                  ...
                  className={`flex-1 rounded-2xl px-4 py-3 text-sm font-bold transition-all ${activeTab === "lectures" ? "bg-slate-900 text-white dark:bg-white dark:text-slate-900 shadow-lg" : "text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800"}`}
                >
                  محاضرات
                </button>
                <button
                  onClick={() => setActiveTab("quizzes")}
                  className={`flex-1 rounded-2xl px-4 py-3 text-sm font-bold transition-all ${activeTab === "quizzes" ? "bg-slate-900 text-white dark:bg-white dark:text-slate-900 shadow-lg" : "text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800"}`}
                >
                  الاختبارات
                </button>
              </div>
                  ...
                  <div className="rounded-[1.75rem] border border-white/70 dark:border-white/10 bg-white/90 dark:bg-slate-900/85 backdrop-blur-xl p-4 shadow-xl space-y-4">
                    <div className="flex items-center justify-between gap-3">
                      <div>
                        ...
                        <p className="text-sm text-slate-500 dark:text-slate-400">واجهة مشاهدة مريحة بأسلوب أقرب إلى يوتيوب مع تجربة أهدأ للعين.</p>
                      </div>
                    </div>
                    {selectedVideo ? (
                      <>
                        {playerLoading && <p className="text-sm text-slate-500 dark:text-slate-400">جارٍ تجهيز رابط آمن للفيديو...</p>}
                        {playerError && <p className="text-sm text-rose-600 dark:text-rose-400">{playerError}</p>}
                        {secureEmbedUrl && (
                          <BunnyPlayer
                            embedUrl={secureEmbedUrl}
                            fallbackEmbedUrl={embedFallbackUrl || undefined}
                        ...
                        <div className="flex flex-wrap items-center justify-between gap-3 rounded-2xl bg-slate-50 dark:bg-slate-800/70 px-4 py-3">
                          <div>
                            ...
                            <p className="text-xs text-slate-500 dark:text-slate-400">بعد المشاهدة يتم تسجيل التقدم داخل الكورس.</p>
                          </div>
                          <button onClick={markVideoWatched} className="px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl text-sm font-bold">
                            تم مشاهدة الفيديو ✅
                          </button>
                        </div>
                      </>
                    ) : (
                      <p className="text-sm text-slate-500 dark:text-slate-400">لا يوجد فيديوهات في هذا الكورس حتى الآن.</p>
                    )}
                  </div>
                </div>
              ) : (
                    ...
                    {selectedQuiz ? (
                      <>
                        <div className="rounded-2xl bg-slate-50 dark:bg-slate-800/70 p-4">
                          ...
                          <p className="text-sm text-slate-600 dark:text-slate-300 mt-2">الاختبار يفتح في صفحة منفصلة مع مؤقت خاص بهذا الاختبار: {selectedQuiz.timeLimitMinutes} دقيقة.</p>
                        </div>
                        <button
                          onClick={() => router.push(`/quizzes/${selectedQuiz.id}`)}
                          className="px-5 py-3 bg-slate-900 hover:bg-slate-700 dark:bg-white dark:text-slate-900 dark:hover:bg-slate-100 text-white rounded-xl font-bold"
                        >
                          ابدأ الاختبار الآن
                        </button>
                      </>
                    ) : (
                      <p className="text-sm text-slate-500 dark:text-slate-400">لا يوجد اختبارات في هذا الكورس حتى الآن.</p>
                    )}
                  </div>
                </div>
              )}
            ...
            <aside className="space-y-4">
              <div className="rounded-[1.75rem] p-4 border border-white/70 dark:border-white/10 bg-white/90 dark:bg-slate-900/85 backdrop-blur-xl shadow-xl">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <span className="text-xs text-slate-500 dark:text-slate-400">{activeTab === "lectures" ? "محاضرات" : "اختبارات"}</span>
                </div>
                            ...
                            if (activeTab === "lectures") {
                              ...
                              return (
                                <button
                                  ...
                                  className={`w-full text-right px-3 py-2 rounded-xl text-sm border transition-all ${
                                    selectedVideoId === video.id
                                      ? "border-sky-500 bg-sky-50 dark:bg-sky-950/30 text-sky-700 dark:text-sky-300"
                                      : "border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-300 hover:border-sky-300 dark:hover:border-sky-500/50"
                                  }`}
                                >
                              ...
                              <button
                                ...
                                className={`w-full text-right px-3 py-2 rounded-xl text-sm border transition-all ${
                                  selectedQuizId === quiz.id
                                    ...
                                    : "border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-300 hover:border-emerald-300 dark:hover:border-emerald-500/50"
                                }`}
                              >
                                📝 {quiz.title}
                              </button>
